...
export default function PaginatedFlashcards({
  status, // "ALL" | "KNOWN" | "ALMOST" | "UNKNOWN" (from parent)
  searchTerm,
  setEditingCard,
  setModalOpen,
  toast,
  ...
  const statusForServer = status === "ALL" ? null : status; // we send KNOWN/ALMOST/UNKNOWN
  ...
  const {
    ...
    variables: { page, limit, status: statusForServer }, // use mapped value
    notifyOnNetworkStatusChange: true,
  });
  ...
  useEffect(() => {
    setPage(1);
    setAllLoaded(false);
    refetchList({ page: 1, limit, status: statusForServer });
  ...
  const handleLoadMore = () => {
    fetchMore({
      variables: { page: page + 1, limit, status: statusForServer },
      updateQuery: (prev, { fetchMoreResult }) => {
        if (!fetchMoreResult || !fetchMoreResult.GetCards) return prev;
  ...
  const handleDelete = async (id) => {
    ...
    try {
          ...
          cache.modify({
            fields: {
              GetCards(
                ...
                const filtered = existingData.flashcards.filter(
                  (fc) => readField("id", fc) !== id
                );
                return {
                  ...existingData,
                  flashcards: filtered,
                  total: Math.max(0, existingData.total - 1),
                };
              },
            },
          });
      ...
      setPage(1);
      setAllLoaded(false);
      await refetchList({ page: 1, limit, status: statusForServer });
    } catch (err) {
      console.error("Delete failed:", err);
      toast.error("Delete failed.");
    }
